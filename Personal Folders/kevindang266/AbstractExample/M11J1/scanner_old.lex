...
ZeroDigit						[0-9]
SingedIntegers					[\-\+]?{ZeroDigit}*
SingedInteger					[\-\+]?{ZeroDigit}+
ExponentPart					(ExponentIndicator)(SingedInteger)
BinaryExponentIndicator			[pP]
SingedIntegerWithHexDigit		[\-\+]?{HexDigit}+
BinaryExponent					(BinaryExponentIndicator)(SingedIntegerWithHexDigit)
HexSignificand					(0[xX])?({HexDigit}*\.)?({HexDigit}+)

/*3.10.2. Floating-Point Literals*/
DecimalFloatingPointLiteral					([\-\+]?[0-9]*\.?[0-9]+([eE][\-\+]?[0-9]+)?.)	
HexadecimalFloatingPointLiteral				(0[xX])?[0-9a-fA-F]*\.?[0-9a-fA-F]+([pP][\-\+]?[0-9a-fA-F]+)?.
...
/* 3.8 Identifier */
/* hiral*/
Identifier               [_/$a-zA-Zα-ωΑ-Ω][_/$a-zA-Zα-ωΑ-Ω0-9]*   
...
{HexadecimalFloatingPointLiteral}				 { yylval.name = yytext; return (int)Tokens.HexadecimalFloatingPointLiteral; }    
