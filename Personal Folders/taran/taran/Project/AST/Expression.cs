...
namespace M11J1.AST
{
    ...
    public class UnaryExpression : Expression
    {
        ...
        public UnaryExpression(Expression lhs, string op)
        {
            _op = op;
            _lhs = lhs;
        }
        public override void Dump(int indent)
        {
            Label(indent, "UnaryExpression {0}\n", _op);
            Type.Dump(indent + 1, "type");
            _lhs.Dump(indent + 1, "lhs");
        }
        ...
        public override void TypeCheck()
        {
            ...
            switch (_op)
            {
                ...
                default:
                    {
                        Console.WriteLine($"Unexpected unary operator '{_op}'\n");
                        throw new Exception("TypeCheck error");
                    }
            }
        }
        public override void GenCode(string file)
        {
            ...
            switch (_op)
            {
                ...
                default:
                    {
                        Console.WriteLine("Unexpected unary operator '{0}'\n", _op);
                        throw new Exception("GenCode error");
                    }
            }
        }
